...
export default function CheckoutPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (!direccion || !/^\d{16}$/.test(tarjeta)) {
            setError('Por favor, completa una dirección válida y una tarjeta de 16 dígitos.');
            return;
        }
        ...
        return (
            <div className="text-center mt-20">
                 <div className="bg-red-100 text-red-800 p-6 rounded-lg shadow-md inline-block">
                    <h1 className="text-2xl font-bold">Hubo un error al procesar tu compra</h1>
                    <p className="mt-2">{error}</p>
                    <button onClick={() => setPurchaseStatus('pending')} className="mt-6 bg-blue-500 text-white py-2 px-6 rounded hover:bg-blue-600">
                        Intentar de nuevo
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="container mx-auto max-w-4xl mt-10">
            <h1 className="text-3xl font-bold mb-8 text-center text-black">Finalizar Compra</h1>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {/* Columna de Resumen */}
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-2xl font-semibold mb-6 border-b pb-4 text-black">Resumen del pedido</h2>
                    {cart.items.map(item => (
                    ...
                    <div className="border-t pt-4 mt-4 space-y-2 text-gray-800">
                        ...
                        <div className="flex justify-between text-xl font-bold mt-2 text-black"><span>Total a pagar:</span> <span>${cart.total.toFixed(2)}</span></div>
                    </div>
                </div>

                {/* Columna de Datos de Envío y Pago */}
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-2xl font-semibold mb-6 text-black">Datos de Envío y Pago</h2>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label htmlFor="direccion" className="block text-sm font-medium text-gray-700">Dirección de envío</label>
                        ...
                        <div>
                            <label htmlFor="tarjeta" className="block text-sm font-medium text-gray-700">Tarjeta de crédito (16 dígitos)</label>
                        ...
                        <button type="submit" disabled={isSubmitting} className="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-400">
                            {isSubmitting ? 'Procesando...' : `Pagar $${cart.total.toFixed(2)}`}
                        </button>
                    </form>
                </div>
            </div>
