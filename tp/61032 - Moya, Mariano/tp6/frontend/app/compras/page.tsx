...
export default function ComprasPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 border rounded shadow bg-white">
      ...
      {compras.length === 0 ? (
        <div className="text-center text-gray-600">No hay compras realizadas.</div>
      ) : (
          ...
          {compras.map(c => (
            <li key={c.id} className="py-4 flex justify-between items-center">
              <div>
                ...
                <div className="text-sm text-gray-600">Fecha: {c.fecha}</div>
                <div className="text-sm text-gray-600">Dirección: {c.direccion}</div>
              </div>
              <div className="text-right">
                <div className="font-bold">Total: ${c.total}</div>
                <div className="text-xs text-gray-500">Envío: ${c.envio}</div>
                <button
